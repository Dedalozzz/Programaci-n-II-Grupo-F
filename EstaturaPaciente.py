...
def guardar_en_archivo():
    with open("pacientesaltura.txt", "w" , encoding="utf-8") as archivo:
        for paciente in pacientes_data:
            archivo.write(f"{paciente['Nombre']}|{paciente['Fecha de nacimiento']}|{paciente['Edad']}|"
             ...
             f"{paciente['Tipo de Seguro']}|{paciente['Centro Medico']}|{paciente['Altura']}\n")            
#lista de pacientes
pacientes_data=[]
def cargar_treeview():
    #limpiar treeview
...
def cargar_desde_archivo_pacientes():
    try:
        with open("pacientesaltura.txt", "r", encoding="utf-8") as archivo:
        ...
        open("pacientesaltura.txt", "w", encoding="utf-8").close()
...
ventanaPrincipal.geometry("920x600")
...
# Altura
labelAltura = tk.Label(framePacientes, text=" Altura (cm):")
labelAltura.grid(row=5, column=2, padx=5, pady=5, sticky="w")
entryAltura = tk.Entry(framePacientes)
entryAltura.grid(row=5, column=3, padx=5, pady=5, sticky="w")
# Tipo de seguro
...
treeview = ttk.Treeview(framePacientes, columns=("Nombre","FechaN","Edad", "Género", "GrupoS", "TipoS", "CentroM", "Altura"), show="headings")
# Definir encabezados
...
treeview.heading("Altura", text="Altura (cm)")
...
treeview.column("Altura", width=80)
